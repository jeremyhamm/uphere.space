...
def subpoint(self, iterations):
  ...
  """
  if self.center != 399:
    raise ValueError("you can only ask for the geographic subpoint of a position measured from Earth's center")
  t = self.t
  xyz_au = einsum('ij...,j...->i...', t.M, self.position.au)
  lat, lon, elevation_m, self.earth_R = reverse_terra(xyz_au, t.gast, iterations)

  from skyfield.toposlib import Topos
  return Topos(latitude=Angle(radians=lat), longitude=Angle(radians=lon), elevation_m=elevation_m)
...
def satellite_visiable_area(earth_radius, satellite_elevation):
  ...
  """
  return ((2 * pi * ( earth_radius ** 2 ) * ( earth_radius + satellite_elevation)) / (earth_radius + earth_radius + satellite_elevation))
...
print('Visible area (m^2)', satellite_visiable_area(geocentric.earth_radius(), geocentric_sub.elevation.m))